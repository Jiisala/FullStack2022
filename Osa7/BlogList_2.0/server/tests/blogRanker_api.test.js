...
describe('tests for saved blogs', () => {
  ...
  describe('posting', () => {
    ...
    test('blogs with incomplete information wont be added', async () => {
      ...
      await api.post('/api/blogs').send(noTitleBlog).set(auth).expect(400)
      await api.post('/api/blogs').send(noAuthorBlog).set(auth).expect(400)
      await api.post('/api/blogs').send(noURLBlog).set(auth).expect(400)
...
describe('Tests for users', () => {
  ...
  test('creation fails with correct statuscode if username is too short', async () => {
    ...
    expect(result.body.error).toContain(
      'username and password must be atleast three characters long'
    )
